<div>
  <div class="row py-2 px-2">
    <div class="col-sm-6 col-md-9">
      <div class="btn-group" role="group" aria-label="Third group" id="onlineoffine">
        <button type="button" class="btn btn-info mr-0 " data-bs-placement="top" title="Online" data-bs-toggle="toggle" id="buttonOnline">
          <i class="fa-solid fa-earth-asia"></i> Online
        </button>
      </div>
      <div class="btn-group" role="group" aria-label="Third group">
        <button type="button" class="btn btn-info mr-0 " data-bs-placement="top" title="Member" data-bs-toggle="toggle" id="buttonMember">
  ...
  <form id="setting-form" method="POST" action="{{route('pelanggan.transaksi.store')}}">
    ...
    `;
        ...
        // onchange
        let tokoCityId = 255; // ID Kota Gresik
        let jawaTimurProvinceId = 11; // ID Provinsi Jawa Timur
        let jawaTimurCities = [255, 456, 789]; // Contoh ID kota di Jawa Timur, tambahkan semua ID kota di Jawa Timur ke dalam array ini
        ...
        getDataCity = async (sel = null) => {
          ...
          // Set default biaya ongkir dalam Gresik
          if (city === tokoCityId) {
            ongkir.value = 5000; // Misalkan biaya ongkir dalam kota adalah 5000
          } else if (jawaTimurCities.includes(parseInt(city))) {
            // Set biaya ongkir lebih murah untuk area Jawa Timur
            await axios.get(`http://127.0.0.1:8000/pelanggan/rajaongkir/cost?provinsi_id=${provinsi_id}&city_id=${city}&courir=${courir}&weight=${weight}`)
              .then(response => {
                let datas = response.data.rajaongkir.results;
                let jawaTimurOngkir = 0;
                datas.forEach(function(data) {
                  console.log(data.name, data.code, data.costs, data.costs[0].cost[0].value);
                  jawaTimurOngkir = data.costs[0].cost[0].value;
                });
                // Misalkan diskon ongkir 20% untuk area Jawa Timur
                ongkir.value = Math.round(jawaTimurOngkir * 0.8);
              })
              .catch(error => console.error(error));
          } else {
            // Panggil API untuk mendapatkan biaya ongkir berdasarkan kota tujuan di luar Jawa Timur
            await axios.get(`http://127.0.0.1:8000/pelanggan/rajaongkir/cost?provinsi_id=${provinsi_id}&city_id=${city}&courir=${courir}&weight=${weight}`)
              .then(response => {
                let datas = response.data.rajaongkir.results;
                datas.forEach(function(data) {
          ...
          let totalbayarNow = parseInt(totalbayarNumber.value) + parseInt(ongkir.value);
